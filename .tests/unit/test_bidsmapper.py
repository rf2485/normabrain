...
def test_bidsmapper(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/bidsmapper/config")
        data_path = Path(".tests/unit/bidsmapper/data")
        expected_path = Path(".tests/unit/bidsmapper/expected")
